...
is_epsilon_variation = False

### Populations being worked with
N_vec, N_vec_vacc, N_vec_vacc_boosted = model_populations(is_vacc)
...
if is_generate_transmission_rates:
    ...
    proportionate_optimisation(N_vec, N_vec_vacc, N_vec_vacc_boosted, is_vacc, time, sigma, gamma)
    
    print('Assortative optimisation')
    # Fit assortative mixing transmission rates
    assortative_optimisation(N_vec, N_vec_vacc, N_vec_vacc_boosted, is_vacc, time, sigma, gamma)
    
    print('non-parametric optimisation')
    # Fit non-parametric mixing transmission rates
    non_parametric_optimisation(N_vec, N_vec_vacc, N_vec_vacc_boosted, is_vacc, time, sigma, gamma)

if is_generate_SEIR_results:
    
    # Proportionate mixing
    run_SEIR_model(N_vec, N_vec_vacc, N_vec_vacc_boosted, is_vacc, time, sigma, gamma, is_prop=True, is_non_parametric=False)
    
    # Assortative mixing
    for is_SA1, is_statsnz in [(False,False),(False,True),(True,True)]:
        run_SEIR_model(N_vec, N_vec_vacc, N_vec_vacc_boosted, is_vacc, time, sigma, gamma, is_prop=False,
...
if is_generate_plots:
    ...
    fig,axs = plt.subplots(2,2,figsize=(6.8*2,4.8*2)) 
    
    contact_vs_reproduction_number(N_vec, N_vec_vacc,N_vec_vacc_boosted,is_SA1,is_statsnz,
                                       is_vacc,is_prop,is_non_parametric, gamma=gamma,ax=axs[0,:],
                                       titles=['Proportionate transmission rates', '$R_0$ (Proportionate)'])
    
    ### Comparrison of contact rates - Assortative vs non parametric
    contact_vs_contact_rates(N_vec, is_SA1s = [False,False],
                             is_statsnzs=[False,False],
                             is_vacc=is_vacc,
                             is_props = [False,False],
                             is_non_parametrics = [False,True],
                             titles=["Assortative","Unconstrained"],
                             ax=axs[1,:], title_letters=['c','d']) # transmission rates will follow
    if is_save_generated_plots:
        save_image('transmission_rates_and_reproduction_number',CAR=None,is_vacc=is_vacc,is_prop=False,is_non_parametric=True,is_SA1=is_SA1,is_statsnz=is_SA1,counterfactual=-1,file_format = 'png')
    
    ### Heat plots
    # ['Proportionate','Assortative','Unconstrained','Per capita proportionate','Per capita assortative','Per capita unconstrained','Proportionate vs assortative', 'Proportionate vs unconstrained','Assortative vs unconstrained']
    ...
    heat_map((beta_prop*N_vec,beta_assort*N_vec,beta_non_para*N_vec,beta_prop,beta_assort,beta_non_para,beta_prop-beta_assort,beta_prop-beta_non_para,beta_assort-beta_non_para),
             ['']*9, is_title_lettered_only=True,
              scaling=[0,0,0,7,7,7,7,7,7],create_fig=True,labels=["Māo","Pac","Asi","Oth"],max_comp='rowwise', is_lower_triangluar=np.array([False,False,False,True,True,True,True,True,True]),cmaps=["viridis","viridis","viridis","viridis","viridis","viridis","RdBu","RdBu","RdBu"])
    if is_save_generated_plots:
    ...
    heat_map((beta_SA1*N_vec,beta_SA2*N_vec,beta_SA1,beta_SA2),['SA1','SA2','Per capita SA1','Per capita SA2'],
              scaling=[0,0,7,7],create_fig=True,labels=["Māo","Pac","Asi","Oth"],max_comp='rowwise',num_cols=2,
              is_title_lettered_only= True)
    if is_save_generated_plots:
